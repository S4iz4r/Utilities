...
if [[ $1 == *.*.*. ]] && [[ $2 ]]; then
    ...
    for host in ${hosts[@]}; do
        if [[ $host != $($net $2 | grep "inet " | awk '{print $2}') ]]; then
    ...
    echo -e "[!] Port scanner range set from 1 to 5000 by deffault"
    echo -e "[!] Example: ./HPDiscovery.sh 127.0.0. eth0"
    echo -e "[!] Example: ./HPDiscovery.sh 127.0.0. eth0 65535"
    echo -e "[!] Example: ./HPDiscovery.sh 127.0.0. eth0 1000 5000\n"
    tput cnorm; exit 1
fi
